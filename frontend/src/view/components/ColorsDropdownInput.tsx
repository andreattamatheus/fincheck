    ...
    <div>
      <DropdownMenu.Root>
        <DropdownMenu.Trigger>
          <button
            className={cn(
              "bg-white rounded-lg w-full border border-gray-500 px-3 h-[52px] text-gray-700",
              "focus:border-gray-800 transition-all outline-none text-left relative",
        ...
        <DropdownMenu.Content className="grid grid-cols-4 z-[55]">
          {colors.map((color) => (
            <DropdownMenu.Item
              key={color.color}
